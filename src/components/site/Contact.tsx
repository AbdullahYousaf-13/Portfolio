...
type Item = { icon: React.ComponentType<{ className?: string }>; label: string; value: string; href: string; copy?: string };

export function Contact() {
  const items: Item[] = [
    { icon: Mail, label: "Email", value: profile.email, href: `mailto:${profile.email}`, copy: profile.email },
    { icon: Phone, label: "Phone", value: profile.phone, href: `tel:${profile.phone.replace(/\s/g, "")}`, copy: profile.phone },
            ...
            return (
              <motion.div key={it.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5, delay: i * 0.05, ease: [0.22, 1, 0.36, 1] }} className="panel group relative flex items-center gap-4 overflow-hidden p-6 transition-transform hover:-translate-y-0.5">
                ...
                <div className="relative min-w-0 flex-1">
                  <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{it.label}</div>
                  <a href={it.href} target={it.href.startsWith("http") ? "_blank" : undefined} rel="noreferrer" className="mt-0.5 block truncate font-display text-base font-medium text-foreground transition-colors hover:text-primary">
                    {it.value}
                  </a>
                </div>
                {it.copy && (
                  <button aria-label={`Copy ${it.label}`} onClick={() => doCopy(it.copy!)} className="relative flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md border border-border bg-surface-2 text-muted-foreground transition-colors hover:border-primary hover:text-primary">
