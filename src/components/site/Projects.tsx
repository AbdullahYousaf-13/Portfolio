            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background:
                      "radial-gradient(600px 200px at 50% 0%, oklch(0.72 0.14 195 / 0.16), transparent 70%)",
                  }}
                />
                <div className="grid grid-cols-1 lg:grid-cols-12">
                  <div className={`relative lg:col-span-6 ${reverse ? "lg:order-2" : ""}`}>
                    <div className="relative h-72 overflow-hidden bg-surface-2 lg:h-full lg:min-h-[420px]">
                      <img
                        ...
                        width={1280}
                        height={800}
                        className={`h-full w-full ${coverClass}`}
                      />
                  ...
                  <div
                    className={`flex flex-col justify-between gap-6 p-7 md:p-9 lg:col-span-6 ${reverse ? "lg:order-1" : ""}`}
                  >
                    <div>
                    ...
                    <div>
                      <div className="flex flex-wrap gap-1.5">
                        {p.stack.map((s) => (
                          <span
                            key={s}
                            className="rounded-md border border-border bg-surface-2 px-2 py-0.5 font-mono text-[10px] text-foreground/80"
                          >
                            {s}
                          </span>
                        ))}
                      ...
                      <div className="mt-5 flex flex-wrap items-center gap-2">
                        {p.links.map((l) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="group/link inline-flex items-center gap-1.5 rounded-full border border-border-strong bg-surface-2 px-3.5 py-2 text-xs font-semibold text-foreground transition-colors hover:border-primary hover:text-primary"
                          >
